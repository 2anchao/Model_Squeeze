...
from utils.kmeans_cluster import QuantConv2d, QuantLinear
    ...
    def __init__(self, inp, oup, kernel_size=3, stride=1, pad=1,use_relu = True):
        ...
        if self.use_relu:
            self.convs = nn.Sequential(
                QuantConv2d(inp, oup, kernel_size, stride, pad, bias=False),
                nn.BatchNorm2d(oup), 
...
class PeleeNet(nn.Module):
    def __init__(self,num_classes=1000, num_init_features=32, growthRate=32, nDenseBlocks = [3,4,8,6], bottleneck_width=[1,2,4,4]):
        ...
        self.classifier = nn.Sequential(
            # nn.Dropout(),
            QuantLinear(total_filter[len(nDenseBlocks)-1], self.num_classes)
        )
...
class Peleenet32_Small(PeleeNet):
    ...
    def use_kmquant(self, quant_bias=False, quant_bit=4):
        for m in self.modules():
            if isinstance(m, QuantConv2d):
                print(m)
                m.kmeans_quant(quant_bias=quant_bias, quant_bit=quant_bit)
            elif isinstance(m, QuantLinear):
                print(m)
                m.kmeans_quant(quant_bias=quant_bias, quant_bit=quant_bit)
